...
import ManagerController, { Controller } from '../helpers/Controller';
...
export default class ResourceController extends ManagerController {
    ...
    private initRoutes() {
        ...
        this.router.post('/:id/attachments/:attachment_reference', idQuery('id', 'attachment_reference'), this.attach);
        this.router.delete('/:id/attachments/:attachment_id', idQuery('id', 'attachment_id'), this.detach);
        this.router.delete('/:id', idQuery('id'), this.delete);
    }
    private uploadImages = async (req: ApiRequest, res: ApiResponse<PublicResource>, next: NextFunction): Promise<void> => {
        const files = req.files;
        if (!files) {
            next(badRequestError({ message: ERRORS.MEDIA_NOT_FOUND }));
            return;
    ...
    private getOne = async (req: ApiRequest<{ id: Types.ObjectId }>, res: ApiResponse<PublicResource>, next: NextFunction): Promise<void> => {
        const { id } = req.body;
        ResourceModel.findById(id).then((resource) => {
            if (!resource) {
                next(badRequestError({ message: ERRORS.NOT_FOUND }));
    ...
    private update = async (req: ApiRequest, res: ApiResponse<PublicResource>, next: NextFunction): Promise<void> => {
        const { id } = req.body;
        ResourceModel.findById(id).then((resource) => {
            if (!resource) {
                next(badRequestError({ message: ERRORS.NOT_FOUND }));
    ...
    private getAttachments = async (req: ApiRequest<{ id: Types.ObjectId }>, res: ApiResponse<Attachment[]>, next: NextFunction): Promise<void> => {
        const { id } = req.body;
        ResourceModel.findById(id).then((resource) => {
            if (!resource) {
                next(badRequestError({ message: ERRORS.NOT_FOUND }));
    ...
    private attach = async (req: ApiRequest<{
        attachment_reference: Types.ObjectId,
        attachment_collection: string,
    } & { id: Types.ObjectId }>, res: ApiResponse<PublicAttachment>, next: NextFunction): Promise<void> => {
        const { id: IdResource, attachment_reference, attachment_collection } = req.body;
    ...
    private detach = async (req: ApiRequest<{ id: Types.ObjectId, attachment_id: Types.ObjectId }>, res: ApiResponse<PublicAttachment>, next: NextFunction): Promise<void> => {
        const { id, attachment_id } = req.body;
        ResourceModel.findById(id).then((resource) => {
            if (!resource) {
                next(badRequestError({ message: ERRORS.NOT_FOUND }));
