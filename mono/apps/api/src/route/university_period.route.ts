...
import UniversityPeriodModel, {
  HydratedUniversityPeriod,
} from '../model/UniversityPeriod.model';
import {
  ApiDeleteResponse,
  ApiRequest,
  ApiResponse,
...
export default class UniversityPeriodController extends ManagerController {
  ...
  private getAll = async (
    ...
    res: ApiResponsePagination<UniversityPeriod>,
    next: NextFunction,
  ): Promise<void> => {
    const { filter, limit, page } = req.body;
    UniversityPeriodModel.paginate(filter, { limit, page })
      .then((result) => {
        if (!result) {
  ...
  private create = async (
    req: ApiRequest,
    res: ApiResponse<PublicUniversityPeriod>,
    next: NextFunction,
  ): Promise<void> => {
  ...
  private getCurrent = async (
    _: ApiRequest,
    res: ApiResponse<PublicUniversityPeriod>,
    next: NextFunction,
  ) => {
    UniversityPeriodModel.findCurrentPeriod()
      .then((result) => {
        if (!result) {
  ...
  private updateCurrent = async (
    req: ApiRequest<Partial<PublicUniversityPeriod>>,
    res: ApiResponse<PublicUniversityPeriod>,
    next: NextFunction,
  ) => {
    UniversityPeriodModel.findCurrentPeriod()
      .then((currentPeriod) => {
        ...
        UniversityPeriodModel.updateCurrentPeriod(req.body)
          .then((result) => {
            if (!result) {
  ...
  private getOne = async (
    ...
    const { id } = req.body;
    UniversityPeriodModel.findById(id)
      .then((result) => {
        if (!result) {
          next(badRequestError({ message: ERRORS.NOT_FOUND }));
          return;
  ...
  private update = async (
    req: ApiRequest<Partial<PublicUniversityPeriod & { id: Types.ObjectId }>>,
    res: ApiResponse<PublicUniversityPeriod>,
    next: NextFunction,
  ) => {
    const { id } = req.body;
    UniversityPeriodModel.findByIdAndUpdate(id, req.body, { new: true })
      .then((result) => {
        if (!result) {
          next(badRequestError({ message: ERRORS.NOT_FOUND }));
          return;
  ...
  private delete = async (
    ...
    const { id } = req.body;
    UniversityPeriodModel.deleteOne({ _id: id })
      .then((result) => {
