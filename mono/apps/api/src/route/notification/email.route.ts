...
router.post(
  '/send',
  async (
    req: ApiRequest<z.infer<typeof zodPostEmail>>,
    res: ApiResponse<EmailResponse>,
  ) => {
    try {
      ...
      transporter.sendMail(mailOptions, (error, info) => {
        if (error) {
          res
            .status(500)
            .json({ status: 'error', message: ERRORS.INTERNAL_SERVER_ERROR });
        } else {
          res.status(200).json({
            status: 'success',
            data: { message: MESSAGE.EMAIL_SENT, info },
          });
        }
      });
    } catch {
