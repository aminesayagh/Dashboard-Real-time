...
import PostulationModel, {
  HydratedPostulation,
} from '../model/Postulation.model';
...
export default class DepartmentController extends ManagerController {
  ...
  private initRoutes() {
    ...
    this.router.get(
      '/:id/postulations/:postulation_id',
      idQuery('id', 'postulation_id'),
      this.getPostulation,
    );
    this.router.put(
      '/:id/postulations/:postulation_id',
      idQuery('id', 'postulation_id'),
      this.updatePostulation,
    );
    this.router.delete(
      '/:id/postulations/:postulation_id',
      idQuery('id', 'postulation_id'),
      this.deletePostulation,
    );
  }
  private getAll = async (
    req: ApiRequest<PaginationBody>,
    res: ApiResponsePagination<Department>,
    next: NextFunction,
  ): Promise<void> => {
    const { filter, limit, page } = req.body;
    DepartmentModel.paginate(filter, { limit, page })
      .then((result) => {
        if (!result) {
  ...
  private create = async (
    req: ApiRequest,
    res: ApiResponse<PublicDepartment>,
    next: NextFunction,
  ): Promise<void> => {
  ...
  private update = async (
    req: ApiRequest<Partial<Department> & { id: Types.ObjectId }>,
    res: ApiResponse<PublicDepartment>,
    next: NextFunction,
  ): Promise<void> => {
    const { id } = req.body;
    DepartmentModel.findByIdAndUpdate(id, req.body, { new: true })
      .then((result) => {
        if (!result) {
          next(badRequestError({ message: ERRORS.NOT_FOUND }));
          return;
  ...
  private getLocations = async (
    req: ApiRequest<PaginationBody & { id: Types.ObjectId }>,
    res: ApiResponsePagination<Location>,
    next: NextFunction,
  ): Promise<void> => {
    const { id } = req.body;
    const { filter, limit, page } = req.body;
    LocationModel.paginate({ ...filter, department_id: id }, { limit, page })
      .then((result) => {
        if (!result) {
          next(badRequestError({ message: ERRORS.BAD_REQUEST }));
          return;
  ...
  private getLocation = async (
    req: ApiRequest<{ id: Types.ObjectId; location_id: Types.ObjectId }>,
    res: ApiResponse<PublicLocation>,
    next: NextFunction,
  ): Promise<void> => {
    const { id, location_id } = req.body;
    LocationModel.findOne({ _id: location_id, department_id: id })
      .then((result) => {
        if (!result) {
          next(badRequestError({ message: ERRORS.NOT_FOUND }));
          return;
  ...
  private updateLocation = async (
    req: ApiRequest<
      Partial<Location> & { id: Types.ObjectId; location_id: Types.ObjectId }
    >,
    res: ApiResponse<PublicLocation>,
    next: NextFunction,
  ): Promise<void> => {
    ...
    LocationModel.findOneAndUpdate(
      { _id: location_id, department_id: id },
      req.body,
      { new: true },
    )
      .then((result) => {
  ...
  private deleteLocation = async (
    ...
    const { id, location_id } = req.body;
    LocationModel.findOneAndDelete({ _id: location_id, department_id: id })
      .then((result) => {
        if (!result) {
          next(badRequestError({ message: ERRORS.NOT_FOUND }));
          return;
  ...
  private getPostulations = async (
    req: ApiRequest<PaginationBody & { id: Types.ObjectId }>,
    res: ApiResponsePagination<Postulation>,
    next: NextFunction,
  ): Promise<void> => {
    const { filter, limit, page, id } = req.body;
    PostulationModel.paginate(
      { ...filter, postulation_department_id: id },
      { limit, page },
    )
      .then((result) => {
        if (!result) {
  ...
  private createPostulation = async (
    req: ApiRequest<Partial<Postulation> & { id: Types.ObjectId }>,
    res: ApiResponse<PublicPostulation>,
    next: NextFunction,
  ): Promise<void> => {
    const { id, ...bodyPostulation } = req.body;
    const postulation = new PostulationModel({
      ...bodyPostulation,
      postulation_department_id: id,
    });
    postulation
      .save()
      .then((result) => {
  ...
  private getPostulation = async (
    req: ApiRequest<{ id: Types.ObjectId; postulation_id: Types.ObjectId }>,
    res: ApiResponse<PublicPostulation>,
    next: NextFunction,
  ): Promise<void> => {
    const { id, postulation_id } = req.body;
    PostulationModel.findOne({
      _id: postulation_id,
      postulation_department_id: id,
    })
      .then((result) => {
  ...
  private updatePostulation = async (
    req: ApiRequest<
      Partial<Postulation> & {
        id: Types.ObjectId;
    ...
    const { id, postulation_id } = req.body;
    PostulationModel.findOneAndUpdate(
      { _id: postulation_id, postulation_department_id: id },
      req.body,
      { new: true },
    )
      .then((result) => {
  ...
  private deletePostulation = async (
    req: ApiRequest<{ id: Types.ObjectId; postulation_id: Types.ObjectId }>,
    res: ApiDeleteResponse,
    next: NextFunction,
  ): Promise<void> => {
    const { id, postulation_id } = req.body;
    PostulationModel.findOneAndDelete({
      _id: postulation_id,
      postulation_department_id: id,
    })
      .then((result) => {
